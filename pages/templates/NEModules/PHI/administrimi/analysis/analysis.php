<?
function analysis_eventHandler($event)
{
	...
	switch ($event->name)
	       {
                    ...
                    IF ($id_pk == "")
                       {
                        //gjenerohet max_id ------------------------------------------------------------------------------
                          $max_id = 0;
                          $sql = "SELECT IF(id_analysis IS NULL, 0, MAX(id_analysis)) as max_id 
                                    FROM phi_analysis 
                                 ";
                        ...
                        //RASTI INSERT -----------------------------------------------------------------------------------
                          $kolonat = "id_analysis,    record_user";
			              ...
			              //formohet inserti -----------------------------------------------------------------------------
                            ...
                            $sql_INS_UPD = "INSERT INTO phi_analysis (".$kolonat.") VALUES(".$vlerat.")";
			              //formohet inserti -----------------------------------------------------------------------------
                        //RASTI INSERT -----------------------------------------------------------------------------------
                       }
                    ELSE
                       {
                        //RASTI UPDATE -----------------------------------------------------------------------------------
                          ...
                          $sql_INS_UPD = "UPDATE phi_analysis SET ".$sql_INS_UPD." WHERE id_analysis = '".ValidateVarFun::f_real_escape_string($id_pk)."'";
                        //RASTI UPDATE -----------------------------------------------------------------------------------
                       }
            ...
            case "del":
                  ...
                  //kontrollohet per te dhena te varura ---------------------------------------------------------------
                    ...
                    $sql = "SELECT count(*) as nr_rreshta_te_lidhura FROM phi_form_notification_laboratory WHERE id_analysis = '".ValidateVarFun::f_real_escape_string($id_pk)."'";
                    $rs  = WebApp::execQuery($sql);
                    IF (!$rs->EOF())
                       {
                  ...
                  IF ($nr_rreshta_te_lidhura == 0)
                     {
                      $sql_del = "DELETE FROM phi_analysis WHERE id_analysis = '".ValidateVarFun::f_real_escape_string($id_pk)."'";
