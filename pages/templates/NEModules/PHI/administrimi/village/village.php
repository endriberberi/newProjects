<?
function village_eventHandler($event)
{
	...
	switch ($event->name)
	       {
                    ...
                    IF ($id_pk == "")
                       {
                        //gjenerohet max_id ------------------------------------------------------------------------------
                          $max_id = 0;
                          $sql = "SELECT IF(id_village IS NULL, 0, MAX(id_village)) as max_id 
                                    FROM phi_village 
                                 ";
                        ...
                        //RASTI INSERT -----------------------------------------------------------------------------------
                          $kolonat = "id_village,    record_user";
			              ...
			              //formohet inserti -----------------------------------------------------------------------------
                            ...
                            $sql_INS_UPD = "INSERT INTO phi_village (".$kolonat.") VALUES(".$vlerat.")";
			              //formohet inserti -----------------------------------------------------------------------------
                        //RASTI INSERT -----------------------------------------------------------------------------------
                       }
                    ELSE
                       {
                        //RASTI UPDATE -----------------------------------------------------------------------------------
                          ...
                          $sql_INS_UPD = "UPDATE phi_village SET ".$sql_INS_UPD." WHERE id_village = '".ValidateVarFun::f_real_escape_string($id_pk)."'";
                        //RASTI UPDATE -----------------------------------------------------------------------------------
                       }
            ...
            case "del":
                  ...
                  //kontrollohet per te dhena te varura ------------------------------------------------------------------
                    ...
                    $sql = "SELECT count(*) as nr_rreshta_te_lidhura FROM phi_branch           WHERE address_id_village = '".ValidateVarFun::f_real_escape_string($id_pk)."'";
                    $rs  = WebApp::execQuery($sql);
                    IF (!$rs->EOF())
                       {
                    ...
                    IF ($nr_rreshta_te_lidhura == 0)
                       {
                        ...
                        $sql = "SELECT count(*) as nr_rreshta_te_lidhura FROM phi_reporting_entity WHERE address_id_village = '".ValidateVarFun::f_real_escape_string($id_pk)."'";
                        $rs  = WebApp::execQuery($sql);
                        IF (!$rs->EOF())
                           {
                            $nr_rreshta_te_lidhura = $rs->Field("nr_rreshta_te_lidhura");
                  ...
                  IF ($nr_rreshta_te_lidhura == 0)
                     {
                      $sql_del = "DELETE FROM phi_village WHERE id_village = '".ValidateVarFun::f_real_escape_string($id_pk)."'";
...
function village_onRender()
{
 ...
 //VARIABLAT E FAQES -------------------------------------------------------------------------------
   ...
   $vars_page_var[] = "s_id_municipality";
   $vars_page_var[] = "s_id_district";
   $vars_page_var[] = "s_id_commune";
