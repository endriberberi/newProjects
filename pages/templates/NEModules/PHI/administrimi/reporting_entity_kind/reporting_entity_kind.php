<?
function reporting_entity_kind_eventHandler($event)
{
	...
	switch ($event->name)
	       {
                    ...
                    IF ($id_pk == "")
                       {
                        //gjenerohet max_id ------------------------------------------------------------------------------
                          $max_id = 0;
                          $sql = "SELECT IF(id_reporting_entity_kind IS NULL, 0, MAX(id_reporting_entity_kind)) as max_id 
                                    FROM phi_reporting_entity_kind 
                                 ";
                        ...
                        //RASTI INSERT -----------------------------------------------------------------------------------
                          $kolonat = "id_reporting_entity_kind,    record_user";
			              ...
			              //formohet inserti -----------------------------------------------------------------------------
                            ...
                            $sql_INS_UPD = "INSERT INTO phi_reporting_entity_kind (".$kolonat.") VALUES(".$vlerat.")";
			              //formohet inserti -----------------------------------------------------------------------------
                        //RASTI INSERT -----------------------------------------------------------------------------------
                       }
                    ELSE
                       {
                        //RASTI UPDATE -----------------------------------------------------------------------------------
                          ...
                          $sql_INS_UPD = "UPDATE phi_reporting_entity_kind SET ".$sql_INS_UPD." WHERE id_reporting_entity_kind = '".ValidateVarFun::f_real_escape_string($id_pk)."'";
                        //RASTI UPDATE -----------------------------------------------------------------------------------
                       }
            ...
            case "del":
                  ...
                  //kontrollohet per te dhena te varura ------------------------------------------------------------------
                    ...
                    $sql = "SELECT count(*) as nr_rreshta_te_lidhura FROM phi_reporting_entity WHERE id_reporting_entity_kind = '".ValidateVarFun::f_real_escape_string($id_pk)."'";
                    $rs  = WebApp::execQuery($sql);
                    IF (!$rs->EOF())
                       {
                  ...
                  IF ($nr_rreshta_te_lidhura == 0)
                     {
                      $sql_del = "DELETE FROM phi_reporting_entity_kind WHERE id_reporting_entity_kind = '".ValidateVarFun::f_real_escape_string($id_pk)."'";
...
function reporting_entity_kind_onRender()
{
 GLOBAL $session, $event, $G_APP_VARS;
 extract($event->args);
