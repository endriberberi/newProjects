<?
function analysis_result_eventHandler($event)
{
	...
	switch ($event->name)
	       {
                    ...
                    IF ($id_pk == "")
                       {
                        //gjenerohet max_id ------------------------------------------------------------------------------
                          $max_id = 0;
                          $sql = "SELECT IF(id_analysis_result IS NULL, 0, MAX(id_analysis_result)) as max_id 
                                    FROM phi_analysis_result 
                                 ";
                        ...
                        //RASTI INSERT -----------------------------------------------------------------------------------
                          $kolonat = "id_analysis_result,    record_user";
			              ...
			              //formohet inserti -----------------------------------------------------------------------------
                            ...
                            $sql_INS_UPD = "INSERT INTO phi_analysis_result (".$kolonat.") VALUES(".$vlerat.")";
			              //formohet inserti -----------------------------------------------------------------------------
                        //RASTI INSERT -----------------------------------------------------------------------------------
                       }
                    ELSE
                       {
                        //RASTI UPDATE -----------------------------------------------------------------------------------
                          ...
                          $sql_INS_UPD = "UPDATE phi_analysis_result SET ".$sql_INS_UPD." WHERE id_analysis_result = '".ValidateVarFun::f_real_escape_string($id_pk)."'";
                        //RASTI UPDATE -----------------------------------------------------------------------------------
                       }
            ...
            case "del":
                  ...
                  //kontrollohet per te dhena te varura ------------------------------------------------------------------
                    ...
                    $sql = "SELECT count(*) as nr_rreshta_te_lidhura FROM phi_form_notification_laboratory WHERE id_analysis_result = '".ValidateVarFun::f_real_escape_string($id_pk)."'";
                    $rs  = WebApp::execQuery($sql);
                    IF (!$rs->EOF())
                       {
                  ...
                  IF ($nr_rreshta_te_lidhura == 0)
                     {
                      $sql_del = "DELETE FROM phi_analysis_result WHERE id_analysis_result = '".ValidateVarFun::f_real_escape_string($id_pk)."'";
...
function analysis_result_onRender()
{
 GLOBAL $session, $event, $G_APP_VARS;
 extract($event->args);
