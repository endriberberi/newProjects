...
IF (ISSET($post_id) AND ($post_id != ""))
   {
    unset($tab);
    $kushti_where        = 'WHERE id_municipality = "'.ValidateVarFun::f_real_escape_string($post_id).'"';
    $tab['tab_name']     = 'phi_municipality';      //*emri i tabeles ku do behet select
    ...
    //kapim id fshatrave me te cilat eshte e lidhur kjo bashki -----------------------------------------------------------
      unset($lov);
      $lov["name"]         = "id_village";
      $lov["obj_or_label"] = "label";
      $lov["only_ids"]     = "Y";
      $lov["filter"]       = 'WHERE id_municipality = "'.ValidateVarFun::f_real_escape_string($post_id).'"';
      $ids_village_arr     = f_app_lov_default_values($lov);
        
      IF (IS_ARRAY($ids_village_arr))
         {
          $ids_village = implode(",", $ids_village_arr);
         }
      ELSE
         {
          $ids_village = "";
...
//LOV --------------------------------------------------------------------------------------------------------------------
  ...
  //ids_village ----------------------------------------------------------------------------------------------------------
    ...
    IF ($ids_village != "")
       {
        $lov["id_select"] = $ids_village;
       }
    ELSE
       {
        $lov["id_select"] = "-1";
       }
    $lov["object_name"]   = "ids_village";
    $lov["valid"]         = "0,0,0,0,0,0";
    $lov["only_options"]  = "Y";
    $lov["obj_or_label"]  = $obj_or_label;
    $lov["null_print"]    = "F";
  
    $lov_ids_village      = f_app_lov_default_values($lov);
  //ids_village ----------------------------------------------------------------------------------------------------------
//LOV --------------------------------------------------------------------------------------------------------------------
...
//Grid_form --------------------------------------------------------------------------------------------------------------
  ...
  $Grid_form['data'][$nr]['for']              = 'id_record_status';
  ...
  $nr = $nr + 1;
  $Grid_form['data'][$nr]['type']             = 'label';
  $Grid_form['data'][$nr]['value']            = '{{ids_village_mesg}}:';
  $Grid_form['data'][$nr]['for']              = 'ids_village';
  $Grid_form['data'][$nr]['id']               = 'ids_village_label';
  $Grid_form['data'][$nr]['other_attributes'] = '';
  $Grid_form['data'][$nr]['width']            = '12';

  $nr = $nr + 1;
  $Grid_form['data'][$nr]['type']             = $obj_type_select_multiple;
  $Grid_form['data'][$nr]['name']             = 'ids_village';
  $Grid_form['data'][$nr]['value']            = $lov_ids_village;
  $Grid_form['data'][$nr]['id']               = 'id_ids_village';
  $Grid_form['data'][$nr]['placeholder']      = '{{type_to_select_mesg}}';
  $Grid_form['data'][$nr]['other_attributes'] = 'valid="0,0,0,0,0,0" etiketa="{{ids_village_mesg}}"';
  $Grid_form['data'][$nr]['width']            = '12';
