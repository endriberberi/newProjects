<?
function doctor_eventHandler($event)
{
	...
	switch ($event->name)
	       {
		    case "save":
                  ...
                  //mbajme datat ne array --------------------------------------------------------------------------------
                    FOR ($i=0; $i < count($kol_array_all); $i++)
                        {
                         //IF (($kol_array_all[$i] != "ids_user_doctor") AND ($kol_array_all[$i] != "ids_village_doctor"))
                         //   {
                  ...
                  //regjistrohen vlerat ne db ----------------------------------------------------------------------------
                    IF ($id_pk == "")
                       {
                        //NE RASTIN E SHTIMIT NE FILLIM VERIFIKOJME NESE DOKTORI ESHTE I RREGJISTRUAR ME PARE ------------
                          //verifikimi emer + mbiemer --------------------------------------------------------------------
                            $doktori_ekziston = 0;
                            $sql = "SELECT COUNT(1) AS doktori_ekziston
                                      FROM phi_doctor 
                                     WHERE first_name = '".ValidateVarFun::f_real_escape_string($kol_val_array["first_name"])."' AND
                                           last_name  = '".ValidateVarFun::f_real_escape_string($kol_val_array["last_name"])."'
                                   ";
                            ...
                            IF (!$rs->EOF())
                               {
                                $doktori_ekziston = $rs->Field("doktori_ekziston");
                               }
                        
                            IF ($doktori_ekziston > 0)
                               {
                                $G_APP_VARS["kodi"]         = "warning";
                                $G_APP_VARS["mesg"]         = "{{doktori_ekziston_em_mesg}} {{ruajtja_deshtoi_mesg}}";
                                $G_APP_VARS["time"]         = "15";
                          ...
                          //verifikimi kod mjeku -------------------------------------------------------------------------
                            IF ($kol_val_array["doctor_no"] != "")
                               {
                                $doktori_ekziston = 0;
                          
                                $sql = "SELECT COUNT(1) AS doktori_ekziston
                                          FROM phi_doctor 
                                         WHERE doctor_no = '".ValidateVarFun::f_real_escape_string($kol_val_array["doctor_no"])."'
                                       ";
                                ...
                                IF ($doktori_ekziston > 0)
                                   {
                                    $G_APP_VARS["kodi"]         = "warning";
                                    $G_APP_VARS["mesg"]         = "{{doktori_ekziston_kod_mesg}} {{ruajtja_deshtoi_mesg}}";
                                    $G_APP_VARS["time"]         = "15";
                        ...
                        //gjenerohet max_id ------------------------------------------------------------------------------
                          $max_id = 0;
                          $sql = "SELECT IF(id_doctor IS NULL, 0, MAX(id_doctor)) as max_id 
                                    FROM phi_doctor 
                                 ";
                          $rs = WebApp::execQuery($sql);
                          IF (!$rs->EOF())
                        ...
                        //RASTI INSERT -----------------------------------------------------------------------------------
                          $kolonat = "id_doctor,    record_user_ins,                    record_timestamp_ins";
			              ...
			              //formohet inserti -----------------------------------------------------------------------------
                            ...
                            $sql_INS_UPD = "INSERT INTO phi_doctor (".$kolonat.") VALUES(".$vlerat.")";
			              //formohet inserti -----------------------------------------------------------------------------
                        //RASTI INSERT -----------------------------------------------------------------------------------
                       }
                    ELSE
                       {
                        //RASTI UPDATE -----------------------------------------------------------------------------------
                          ...
                          $sql_INS_UPD = "UPDATE phi_doctor SET ".$sql_INS_UPD." WHERE id_doctor = '".ValidateVarFun::f_real_escape_string($id_pk)."'";
                        //RASTI UPDATE -----------------------------------------------------------------------------------
                       }
            ...
            case "del":
                  ...
                  //kontrollohet per te dhena te varura ------------------------------------------------------------------
                    ...
                    $sql = "SELECT count(*) as nr_rreshta_te_lidhura FROM phi_form_notification WHERE id_doctor = '".ValidateVarFun::f_real_escape_string($id_pk)."'";
                    $rs  = WebApp::execQuery($sql);
                    IF (!$rs->EOF())
                       {
                  ...
                  IF ($nr_rreshta_te_lidhura == 0)
                     {
                      $sql_del = "DELETE FROM phi_doctor  WHERE id_doctor = '".ValidateVarFun::f_real_escape_string($id_pk)."'";
...
function doctor_onRender()
{
 ...
 //VARIABLAT E FAQES -------------------------------------------------------------------------------
   ...
   $vars_page_var[] = "s_id_work_position";
   $vars_page_var[] = "s_first_name";
   $vars_page_var[] = "s_last_name";
   $vars_page_var[] = "s_doctor_no";
