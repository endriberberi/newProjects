...
//kushti i kerkimit ------------------------------------------------------------------------------------------------------
  ...
  IF (ISSET($G_APP_VARS['s_address_id_municipality']) AND ($G_APP_VARS['s_address_id_municipality'] != ''))
     {
      $kushti_kerkim .= ' AND address_id_municipality = "'.ValidateVarFun::f_real_escape_string($G_APP_VARS['s_address_id_municipality']).'" ';
     }
  ...
  IF ($nr_rec_total > 0)
     {
      ...
      $kolonat_fusha[] = "id_branch";               $kolonat_etiketa[] = WebApp::getVar("id_mesg");
      $kolonat_fusha[] = "name";                    $kolonat_etiketa[] = WebApp::getVar("emertimi_mesg");
      $kolonat_fusha[] = "address_id_municipality"; $kolonat_etiketa[] = WebApp::getVar("address_id_municipality_mesg");
      
      IF ($exp_data == "Y")
         {
          $kolonat_fusha[] = "address_id_village";  $kolonat_etiketa[] = WebApp::getVar("address_id_village_mesg");
          $kolonat_fusha[] = "address";             $kolonat_etiketa[] = WebApp::getVar("address_mesg");
         }

      $kolonat_fusha[] = "tel";                     $kolonat_etiketa[] = WebApp::getVar("tel_mesg");
      $kolonat_fusha[] = "mobile";                  $kolonat_etiketa[] = WebApp::getVar("mobile_mesg");
      $kolonat_fusha[] = "email";                   $kolonat_etiketa[] = WebApp::getVar("email_mesg");
  ...
  IF ($nr_rec_total > 0)
     {
      ...
      //futemi ne kursorin e te dhenave ----------------------------------------------------------------------------------
        $sql = 'SELECT id_branch                                                         as id_sel,
                       IF (name                    IS NULL, "", name)                    as name_sel,
                       IF (address_id_municipality IS NULL, "", address_id_municipality) as address_id_municipality_sel,
                       IF (address_id_village      IS NULL, "", address_id_village)      as address_id_village_sel,
                       IF (address                 IS NULL, "", address)                 as address_sel,

                       IF (tel                     IS NULL, "", tel)    as tel_sel,
                       IF (mobile                  IS NULL, "", mobile) as mobile_sel,
                       IF (email                   IS NULL, "", email)  as email_sel,
