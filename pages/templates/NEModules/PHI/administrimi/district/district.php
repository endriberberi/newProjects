<?
function district_eventHandler($event)
{
	...
	switch ($event->name)
	       {
                    ...
                    IF ($id_pk == "")
                       {
                        //gjenerohet max_id ------------------------------------------------------------------------------
                          $max_id = 0;
                          $sql = "SELECT IF(id_district IS NULL, 0, MAX(id_district)) as max_id 
                                    FROM phi_district 
                                 ";
                        ...
                        //RASTI INSERT -----------------------------------------------------------------------------------
                          $kolonat = "id_district,    record_user";
			              ...
			              //formohet inserti -----------------------------------------------------------------------------
                            ...
                            $sql_INS_UPD = "INSERT INTO phi_district (".$kolonat.") VALUES(".$vlerat.")";
			              //formohet inserti -----------------------------------------------------------------------------
                        //RASTI INSERT -----------------------------------------------------------------------------------
                       }
                    ELSE
                       {
                        //RASTI UPDATE -----------------------------------------------------------------------------------
                          ...
                          $sql_INS_UPD = "UPDATE phi_district SET ".$sql_INS_UPD." WHERE id_district = '".ValidateVarFun::f_real_escape_string($id_pk)."'";
                        //RASTI UPDATE -----------------------------------------------------------------------------------
                       }
            ...
            case "del":
                  ...
                  //kontrollohet per te dhena te varura ------------------------------------------------------------------
                    ...
                    $sql = "SELECT count(*) as nr_rreshta_te_lidhura FROM phi_village WHERE id_district = '".ValidateVarFun::f_real_escape_string($id_pk)."'";
                    $rs  = WebApp::execQuery($sql);
                    IF (!$rs->EOF())
                       {
                  ...
                  IF ($nr_rreshta_te_lidhura == 0)
                     {
                      $sql_del = "DELETE FROM phi_district WHERE id_district = '".ValidateVarFun::f_real_escape_string($id_pk)."'";
