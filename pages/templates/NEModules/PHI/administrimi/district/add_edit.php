...
IF (ISSET($post_id) AND ($post_id != ""))
   {
    unset($tab);
    $kushti_where        = 'WHERE id_district = "'.ValidateVarFun::f_real_escape_string($post_id).'"';
    $tab['tab_name']     = 'phi_district';            //*emri i tabeles ku do behet select
...
//LOV --------------------------------------------------------------------------------------------------------------------
  //id_region --------------------------------------------------------------------------------------------------------
    unset($lov);
    $lov["name"]          = "id_region";
    IF ($val_rec['id_region'][0]['vl'] != "")
       {
        $lov["id_select"] = $val_rec['id_region'][0]['vl'];
       }
    ELSE
       {
        $lov["id_select"] = "-1";
       }
    $lov["object_name"]   = "id_region";
    $lov["valid"]         = "1,0,0,0,0,0";
    $lov["only_options"]  = "Y";
    $lov["obj_or_label"]  = $obj_or_label;
  
    $lov_id_region        = f_app_lov_default_values($lov);
  //record_status --------------------------------------------------------------------------------------------------------
...
//Grid_form -----------------------------------------------------------------------------------------------------------
  ...
  $Grid_form['data'][$nr]['value']            = $yll.'{{id_region_mesg}}';
  $Grid_form['data'][$nr]['for']              = 'id_region';
  $Grid_form['data'][$nr]['id']               = 'id_id_region_label';
  $Grid_form['data'][$nr]['other_attributes'] = '';
  $Grid_form['data'][$nr]['width']            = $width_lab;

  $nr = $nr + 1;
  $Grid_form['data'][$nr]['type']             = $obj_type_select;
  $Grid_form['data'][$nr]['name']             = 'id_region';
  $Grid_form['data'][$nr]['value']            = $lov_id_region;
  $Grid_form['data'][$nr]['id']               = 'id_id_region';
  $Grid_form['data'][$nr]['placeholder']      = '';
  $Grid_form['data'][$nr]['other_attributes'] = 'valid="1,0,0,0,0,0" etiketa="{{id_region_mesg}}"';
