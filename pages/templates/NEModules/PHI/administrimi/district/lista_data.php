...
//kushti i kerkimit ------------------------------------------------------------------------------------------------------
  ...
  IF (ISSET($G_APP_VARS['s_id_region']) AND ($G_APP_VARS['s_id_region'] != ''))
     {
      $kushti_kerkim .= ' AND id_region = "'.ValidateVarFun::f_real_escape_string($G_APP_VARS['s_id_region']).'" ';
     }
  ...
  IF ($nr_rec_total > 0)
     {
      ...
      $kolonat_fusha[] = "id_district";   $kolonat_etiketa[] = WebApp::getVar("id_mesg");
      $kolonat_fusha[] = "id_region";     $kolonat_etiketa[] = WebApp::getVar("id_region_mesg");
  ...
  IF ($nr_rec_total > 0)
     {
      ...
      //id_region_data -----------------------------------------------------------------------------------------------
        ...
        $lov["obj_or_label"]   = "label";
        $lov["all_data_array"] = "Y";
        $id_region_arr         = f_app_lov_default_values($lov);
      //id_region_data -----------------------------------------------------------------------------------------------------------
      ...
      //futemi ne kursorin e te dhenave ----------------------------------------------------------------------------------
        $sql = 'SELECT id_district                                   as id_sel,
                       IF (id_region     IS NULL, "", id_region)     as id_region_sel,
        ...
        WHILE (!$rs_list->EOF())
	          {
               ...
               $id_region     = $rs_list->Field('id_region_sel');
               $name          = $rs_list->Field('name');
               $record_status = $rs_list->Field('record_status');
               
               //id_region -------------------------------------------------------------------------------------------
                 $lov_id_region = $id_region_arr[$id_region];
               //id_region -------------------------------------------------------------------------------------------
