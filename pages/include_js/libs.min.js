//@prepros-append core/libraries/jquery.min.js
//@prepros-append core/libraries/bootstrap.min.js

// PLUGINS
//@prepros-append plugins/ui/nicescroll.min.js
//@prepros-append plugins/ui/drilldown.js
//@prepros-append plugins/ui/ripple.min.js
...
var dependencies = (function(){
	...
	function _loadScript(options){
		...
		var x = $.getScript(options.url, function() {
		});
		return x;
	}
	function _loadCss(options){
		...
		options.url = options.url || '';
		options.url = config.baseUrl + options.url;
		var x = $.get(options.url, function(){
			$('<link>', {rel:'stylesheet', type:'text/css', 'href':options.url, 'id':options.definition}).appendTo('head');
		});
		return x;
	}

	function _arrayFunct(assetsArray, module){
		var deferreds=[];

		$.each(assetsArray, function(index, val) {
			 /* iterate through array or object */
			var depName = val;

			if (typeof loadedDep[depName] === 'undefined') {

		    	var item = dependencies.get(depName);

				if(item.type === 'script'){
					var defpathArray = item.definition.split('.');
					var defpath;
					$(defpathArray).each(function(index, el){
				    	if(index === 0){
							defpath = window[el];
						}
						else{
						...
						if(typeof defpath === 'undefined'){
							return false;
						}
					});
			    	if(typeof defpath === 'undefined'){
						// Create promise to be reported
						var reportedDefferedScript = $.Deferred();
				  		loadedDep[depName] = {
				  			url: item.url,
				  			deferred: reportedDefferedScript
				  		}
						deferreds.push(reportedDefferedScript);

						// Load dependency after its own dependencies are loaded
						_checkInterdependencies(item, function(moduleitem){
				    		var scriptDeferred = _loadScript({url: item.url});
							scriptDeferred.done(function(){
					    		item.loaded = true;
					    		reportedDefferedScript.resolve();
							})
						})
			    	}
			    }
				else if(item.type === 'css'){
					//if(!loadedDep.includes(item.url)){
					// Create promise to be reported
					var reportedDefferedCSS = $.Deferred();
			  		loadedDep[depName] = {
			  			url: item.url,
			  			deferred: reportedDefferedCSS
			  		}
					deferreds.push(reportedDefferedCSS);

					// Load dependency after its own dependencies are loaded
					_checkInterdependencies(item, function(moduleitem){
				    	var cssDeferred = _loadCss({url: item.url, definition: item.definition});
						//deferreds.push(cssDeferred);
						cssDeferred.done(function(){
							item.loaded = true;
							reportedDefferedCSS.resolve();
						})
				...
				deferreds.push(loadedDep[depName].deferred);
			}
		});

		return deferreds;
	}
	function _checkInterdependencies(item, fn){
		if (typeof item.dependencies !== 'undefined') {
            $.when.apply(null, dependencies.arrayFunct(item.dependencies))
            .done(function(){
                fn(item);
            })
        }else{
	...
	function _get(lib){
		var deptoreturn = null;
		if(typeof depregistry[lib] !== 'undefined'){
			deptoreturn = depregistry[lib];
		}
		return deptoreturn;
	}
