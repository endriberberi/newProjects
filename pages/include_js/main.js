...
jQuery(function () {
	...
	dependencies.init({
		...
		'imagemapster': {
			url: 'imagemapster/imagemapster.min.js',
			type: 'script',
			loaded: false,
			definition: '$.fn.mapster'
		},
		'dcDrilldown': {
			url: '',
			type: 'script',
			loaded: false,
			definition: '$.fn.dcDrilldown'
		}
...
EW.guess_by_term = {
	...
	init: function (u) {
		var module = this;
		var inp = u && u instanceof jQuery ? u.find(".guess_by_term") : $(".guess_by_term");

		var search = $('form.guess_by_form')
		...
		var searchdimm = $('.search-dim');
		var searchsuggest = $('.search-suggest');
		var searchsuggestWrapper = searchsuggest.find('.search-suggest-wrapper');
		var searchbox = $('#navbar-search');
		var body = $('body');
		var animInClass = 'slideInDown';
		var animOutClass = 'slideOutUp';
		var animationEnd = 'webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend';
		...
		$('body').on("click", '.search-close', function(event) { 
		   module.hideSearch(input,searchdimm,searchsuggestWrapper,searchsuggest,searchbox,body,animInClass,animOutClass,animationEnd);
		});
		...
		/*$('body').keypress(function(event) {
		    ...
		    if (event.keyCode == 13 || event.which == 13) {
		        ...
		        $('#guess_by_term').submit();
		    }
		});*/

		
	},
	srch: function (input, idstemp, target, loader,body) {
	...
	hideSearch: function (input,searchdimm,searchsuggestWrapper,searchsuggest,searchbox,body,animInClass,animOutClass,animationEnd) {
		input.val('');

			body.addClass('search-closing');
			var ismobile = $(window).width() < EW.config.settings.brakepoint ? true : false;
			if (ismobile) {
				searchbox.animateCss(animOutClass).one(animationEnd, function () {
					searchbox.hide();
				});
			}
			searchsuggest.animateCss('fadeOut');
			searchdimm.animateCss('fadeOut').one(animationEnd, function () {
				body.removeClass('search-active search-closing has-search-results');
				searchsuggestWrapper.empty();
			});
	}
}
...
/*EW['scrollspy'] = {
    settings: {
        spyselector: '[data-scrollspy]',
        scrollselector: '.nav > li > a',
        headerselectors: '#primary-navbar, #secondary-navbar',
        primaryNavSelectors: '#primary-navbar',
        secondaryNavSelectors: '#secondary-navbar',
        pageTitleSelector: '#page-title',
        contentContainerSelector: '.page-container:first',
        sidebarSel: '#left-navigation-collapse',
        defaultOffset: 50,
        scrollduration: 500
    },
    init: function () {
        ...
        var primaryNavElement = $(settings.primaryNavSelectors);
        // Calculate priimary navbar element height if found
        var primaryNavElementHeight = primaryNavElement.length ? primaryNavElement.height() : 0;
        var secondaryNavElement = $(settings.secondaryNavSelectors);
        // Calculate secondary navbar element height if found and if visible
        var secondaryNavElementHeight = secondaryNavElement.length && secondaryNavElement.is(':visible') ? secondaryNavElement.height() : 0;
        var pageTitleElement = $(settings.pageTitleSelector);
        // Calculate page title element height if found
        var pageTitleElementHeight = pageTitleElement.length ? pageTitleElement.height() : 0;
        var contentContainerPaddingTop = $(settings.contentContainerSelector).length ? parseFloat($(settings.contentContainerSelector).css('padding-top')) : 0;
        var offset = primaryNavElementHeight + secondaryNavElementHeight + pageTitleElementHeight + 10;
        // Init ScrollSpy
        $('body').scrollspy({ target: settings.spyselector, offset: offset });
        $('body').on('click', settings.spyselector + ' ' + settings.scrollselector, function(event){
            ...
            var handleAnimation = function(callback){
                // If in mobile, wait for sliding animation to end before starting scrolling
                if ($(window).width() < EW.config.settings.brakepoint && trigger.parents(settings.sidebarSel + ':first').length) {
                    $(settings.sidebarSel).one(animationEnd, function() {
                        callback();
                    });
                }
                // Otherwise start scrolling imediately
                else{
                    callback();
                }
            }
            if (element.length) {
                handleAnimation(function(){
                    var isfirst = false;
                    // Case when clicking the first element it will scroll top of page might become problematic
                    if (trigger.parent().prev().length === 0) {
                        var scrollTo = 0;
                        // Manually set active class because is not being interpreted correctly from scorllspy
                        isfirst = function(){
                            trigger.parent().addClass('active')
                        }
                    }
                    // Case when scrolling up in mobile devices, requires considering also the title box as it will show
                    else if (element.offset().top < $(window).scrollTop() && $(window).width() < EW.config.settings.brakepoint) {
                        var scrollTo = element.offset().top - primaryNavElementHeight - secondaryNavElementHeight - pageTitleElementHeight - contentContainerPaddingTop;
                    }
                    // All other cases
                    else{
                        var scrollTo = element.offset().top - primaryNavElementHeight - secondaryNavElementHeight - contentContainerPaddingTop;
                    }
                    module.scrollTo(scrollTo, isfirst);
                })
            }
        });
    },
    scrollTo(scrollto, first){
        ...
        if (first) {
            scrollconfig.onAfter = function(){first()}
        }
        $.scrollTo(scrollto, settings.scrollduration, scrollconfig);
    },
    scroll: function(elementY) {
        ...
        window.requestAnimationFrame(function step(timestamp) {
            ...
            var percent = Math.min(time / settings.scrollduration, 1)
            window.scrollTo(0, startingY + diff * percent)
            // Proceed with animation as long as we wanted it to.
            if (time < settings.scrollduration) {
...
EW["search-module"] = {
	init: function () {
		...
		var searchbox = $('#navbar-search');
		var searchinput = searchbox.find('input:first');
		var searchtrigger = $('[data-target="#navbar-search"]');
		var searchdimm = $('.search-dim');
		var searchsuggest = $('.search-suggest');
		var searchsuggestWrapper = searchsuggest.find('.search-suggest-wrapper');
		var body = $('body');

		var searchUrl = searchbox.attr('data-search-url');

		/**
		 * Attached Event Handlers
		 */
		searchbox.on('focus', 'input', function (event) {
			event.preventDefault();
		...
		searchtrigger.on('click', function (event) {
			...
			showSearch();
			body.addClass('search-active');
			//console.log('search trigger clicked', bowser);
			if (bowser.ios) {
				searchinput.focus();
			}
		});
		...
		searchbox.on('input', searchinput, function (event) {
			event.preventDefault();
			body.addClass('has-search-results');
			$.ajax({
				url: searchUrl,
				dataType: 'html',
				beforeSend: function () {
				...
				success: function (data, textStatus, jqXHR) {
					...
					searchsuggestWrapper.html(data);
				}
			});
		});

		function showSearch() {
			searchdimm.animateCss('fadeIn');
			var ismobile = $(window).width() < EW.config.settings.brakepoint ? true : false;
			if (ismobile) {
				searchbox.animateCss(animInClass).one(animationEnd, function () {
					searchinput.focus();
				});
				searchbox.show();
			}
...
EW.modal = {
    ...
    update: function(newdata){
        ...
        console.log();
        if (trigger.is('[data-modal-width]') && trigger.attr('data-modal-width') !== '') {
        	var modalWidth = parseInt(trigger.attr('data-modal-width'));
        	if (isNaN(modalWidth) === false && modalWidth > 0) {
        		if (modalWidth > ($(window).width() - 60) ) {
        			modalWidth = ($(window).width() - 60);
        		}
	        	module.modal.find('.modal-dialog:first').width(modalWidth);
        	}
        }
    ...
    reset: function(){
        ...
        module.modal.find('.modal-dialog:first').css('width','');
        module.modal.removeClass('frame-modal');
    },
    load: function(){
        var module = this;
        var settings = module.settings;
        ...
        if(trigger.is('[data-modal-iframe="true"]')){
        	var iframe = $('<iframe class="modal-frame" src="' + trigger.attr('data-url') + '" allowfullscreen="" frameBorder="0"></iframe>')
        	//iframe.attr('src', trigger.attr('data-url'));
        	console.log('data-modal-iframe="true"', iframe, module.modal.find('.modal-header').height());
        	var iframeheight = 300;
        	if (trigger.is('[data-modal-height]')) {
        		iframeheight = trigger.attr('data-modal-height');

        		iframeheightLimit = $(window).height() - 120;

        		if (iframeheight > iframeheightLimit ) {
        			iframeheight = iframeheightLimit;
        		}
        	}
        	iframe.attr('height', iframeheight);

        	module.modal.addClass('frame-modal');
        	module.content.html(iframe);
        }
        else{
...
EW['asesment-timer'] = {
    ...
    init: function (autostart) {
        var module = this;
        var item = $('#timer-visual');
        var spenttime = parseInt(item.attr('data-timer'));
        var totaltime = parseInt(item.attr('data-timer-total'));
        var remainingtime = totaltime - spenttime;
        var automaticstart = false;
        var timerup = false;
        if (item.is('[data-timer-up="true"]')) {
            timerup = true;
        }

        //data-timer-up

        var continuousreport = item.is('[data-continous-report]');
        var continousReportRate = module.settings.reportrate;
        if (item.is('[data-continous-report-rate]')) {
            continousReportRate = parseFloat(item.attr('data-continous-report-rate'));
        }

        var shownotifications = item.is('[data-show-notifications="true"]');

        var notificationFirst = false;
        var notificationSecond = false;
        var notificationLast = false;

        if (item.is('[data-notifications-first]')) {
            notificationFirst = item.attr('data-notifications-first');
        }
        if (item.is('[data-notifications-second]')) {
            notificationSecond = item.attr('data-notifications-second');
        }
        if (item.is('[data-notifications-last]')) {
            notificationLast = item.attr('data-notifications-last');
        }


        /*//console.log(item);
        //console.log(item.is('[data-notifications-first]'));
        //console.log(item.is('[data-notifications-second]'));
        //console.log(item.is('[data-notifications-last]'));*/

        /**



         * Check if timer should start automatically
         */
        if (jQuery.type(autostart) !== 'undefined') {
            automaticstart = false;
        } else if (autostart === true) {
            automaticstart = true;
        }else if (item.is('[data-target]')) {
            var target = $(item.attr('data-target'));
            if (target.is('[data-has-timer="#'+ item.attr('id') +'"]') && target.is('[disabled]') !== true) {
                automaticstart = true;
            }
        }
        ...
        var timeroptions = {
            ...
            //layout: '<div class="timer-inner"><div class="minutes time-item"><span class="time-value">{mnn}</span><span class="time-label">{ml}</span></div><div class="separator">{sep}<span class="time-label">&nbsp;</span></div><div class="seconds time-item"><span class="time-value">{snn}</span><span class="time-label">{sl}</span></div></div>',
            onExpiry: function(){
                if (item.is('[data-finalize-target]')) {
                    PNotify.removeAll();
                    finalizeTrigger = $(item.attr('data-finalize-target'));
                    finalizeTrigger.trigger('click');
                }
            },
            onTick: function(periods){
                ////console.log(periods);
                ////console.log('module.convertTime(item)', module.convertTime(item));

                //var total = module.convertTime(item);
                var total = $.countdown.periodsToSeconds(periods);

                if (continuousreport) {
                    if (total % continousReportRate === 0) {
                        continousReportTrigger = $(item.attr('data-continous-report'));
                        if (continousReportTrigger.length) {
                            var spenttimecurrent = module.getSpentTime(item);
                            /*//console.log('totaltime', totaltime)
                            //console.log('total', total)
                            //console.log('spenttimecurrent', spenttimecurrent)*/
                            ...
                            //continousReportRate = parseFloat(item.attr('data-continous-report-rate'));
                            continousReportTrigger.attr('data-url', continousReportUrlBefore + spenttimecurrent + continousReportUrlAfter);
                            continousReportTrigger.trigger('click');
                        }
                    }
                }
                if (shownotifications && $.countdown.periodsToSeconds(periods) != remainingtime) {
                    if (total <= module.settings.firstwarning && total >= module.settings.firstwarning - 2 && total > module.settings.secondwarning && item.hasClass(module.settings.firstwarningclass) === false) {
                        item.addClass(module.settings.firstwarningclass);
                        var timershown = Math.round(total/60);
                        var messagetoshow = 'Time will end in approximately '+timershown+' minutes';
                        if (notificationFirst) {
                            messagetoshow = notificationFirst.replace(module.settings.messagestrtag, timershown.toString());
                        }
                        module.showNotify(messagetoshow);
                    }else if (total <= module.settings.secondwarning && total >= module.settings.secondwarning - 2 && total > module.settings.lastwarning && item.hasClass(module.settings.secondwarningclass) === false) {
                        item.addClass(module.settings.secondwarningclass);
                        var timershown = Math.round(total/60);
                        var messagetoshow = 'Time will end in approximately '+timershown+' minutes';
                        if (notificationSecond) {
                            messagetoshow = notificationSecond.replace(module.settings.messagestrtag, timershown.toString());
                        }
                        module.showNotify(messagetoshow, 'error');
                    }else if (total <= module.settings.lastwarning) {
                        item.addClass(module.settings.secondwarningclass);
                        var messagetoshow = 'This examination will close automatically in '+total+' seconds';
                        if (notificationLast) {
                            messagetoshow = notificationLast.replace(module.settings.messagestrtag, total.toString());
                        }
                        module.showNotify(messagetoshow, 'error', total*1000, 'isupdate');
                    }
                }
        ...
        if (timerup) {
            timeroptions.since = spenttime;
        }else{
            timeroptions.until = totaltime - spenttime;
        }

        /**
         * Initialize timer
         */
        var asesmentTimer = item.countdown(timeroptions);

        /**
         * Stop timer if autostart is set to false
         */
        if (!automaticstart) {
            item.countdown('pause');
        }


    },
    convertTime: function(item){
        var remainingtime = $.countdown.periodsToSeconds(item.countdown('getTimes'));
        return remainingtime;
    },
    getTime: function(item){
        var timeset = $.countdown.periodsToSeconds(item.countdown('getTimes'));
        return timeset;
    },
    getSpentTime: function(item){
        var totaltime = parseInt(item.attr('data-timer-total'));
        var timerup = false;
        if (item.is('[data-timer-up="true"]')) {
            timerup = true;
        ...
        if (timerup) {
            var timetoreturn = $.countdown.periodsToSeconds(item.countdown('getTimes'));

        }else{
            var timetoreturn = totaltime - $.countdown.periodsToSeconds(item.countdown('getTimes'));
        }
        return timetoreturn;
    },
    showNotify: function(messageText, messageType, setDuration, isupdate){
        var module = this;
        var notifyOptions = {
            title: messageText,
            animate_speed: 'fast',
            icon: 'fa fa-fw fa-2x fa-warning',
            delay: 3000,
            buttons: {
                closer: false,
                sticker: false
        ...
        if (typeof messageType !== 'undefined') {
            notifyOptions.type = messageType;
        }

        if (typeof setDuration !== 'undefined') {
            notifyOptions.delay = setDuration;
        }

        if (typeof isupdate !== 'undefined') {
            if (module.notify) {
                module.notify.update(notifyOptions);
            }else{
                module.notify = new PNotify(notifyOptions);
            }
        }else{
...
EW['video-player-embed'] = {
	...
	settings: {
		...
		playerHtml: '<video class="video-js vjs-default-skin" controls preload="none" width="100%" height="100%"></video>'
	},
	init: function (wrapper) {
		var module = this;
	...
	setup: function (item) {
		...
		var identifier = settings.identifierPrefix;
...
EW['video-player'] = {
	dependencies: ['bowser', 'jqueryui'],
	settings: {
		videoviewerPluginUrl: APP_URL + 'plugins/videojs/index.html?file=',
		modalwidth: 802
	},
	player: null,
	init: function (b) {
		...
		var modaliconvideo = '<i class="icon-play" class="text-info" style="margin-right: 5px;"></i>';
		var modaliconaudio = '<i class="icon-play" class="text-info" style="margin-right: 5px;"></i>';
		...
		$('body').on('click', 'a.view-video, a.view-audio', function (e) {
			...
			var trigger = $(e.currentTarget);
			//openViewer(trigger);
			console.log('view-video triggered');
		});
		...
		function openViewer(triggerElement) {
			...
			if (bowser.tablet === true || bowser.mobile === true) {
				...
				var newwin = window.open(settings.videoviewerPluginUrl + newsource.src + '&poster=' + newsource.poster + '&autoplay=true', '_blank');
			...
			var viewer = '<iframe id="videoViewer" src="' + settings.videoviewerPluginUrl + newsource.src + '&poster=' + newsource.poster + '&autoplay=true" style="border: 0;" width="100%" height="100%" allowfullscreen></iframe>';
			viewer = $(viewer);
	...
	getSource: function (item) {
		...
		if (item.is('[href]') && item.attr('href') !== '') {
			src = item.attr('href');
			src = src.indexOf('?') !== -1 ? src + '&uni=' + uniqueid : src + '?uni=' + uniqueid;
		}

		var poster = '';
		if (item.is('[data-poster]') && item.attr('data-poster') !== '') {
			poster = item.attr('data-poster');
			poster = poster.indexOf('?') !== -1 ? poster + '&uni=' + uniqueid : poster + '?uni=' + uniqueid;
		}
...
EW['view-virtual-slide'] = {
	...
	init: function (b) {
		...
		var modalicon = '<i class="icon-presentation" class="text-info" style="margin-right: 5px;"></i>';
		var defaultTitle = 'View Video';
...
EW['ref-link-viewer'] = {
	...
	init: function (b) {
		...
		var modalicon = '<i class="icon-file-text" class="text-info" style="margin-right: 5px;"></i>';
...
EW['image-html-viewer'] = {
	settings: {
		imageViewerPluginUrl: APP_URL + 'plugins/imageviewer/index.html?file=',
		modalwidth: 600,
		modalMaxHeight: 600
	},
	init: function (b) {
		...
		function openViewer(triggerElement) {
			...
			if (bowser.tablet === true || bowser.mobile === true) {
				...
				var viewer = window.open(settings.imageViewerPluginUrl + fileurl, '_blank');
			...
			var viewer = '<iframe id="imageViewer"  src="' + settings.imageViewerPluginUrl + fileurl + '" style="border: 0;" width="100%" height="100%" allowfullscreen></iframe>';
			viewer = $(viewer);
			modalwin.html(viewer);
...
EW.datepicker = {
	...
	init: function () {
		...
		items.each(function (index, el) {
			...
			var pickeritem = item.datepicker(pickeroptions);
			console.log('datepicker item', item, pickeritem);
		});
	}
};
...
EW["handle-preview-layout"] = {
    ...
    init: function () {
        var settings = EW["handle-preview-layout"].settings;

        var win = $(window);

        var module = $(settings.selector);
        var header = $(settings.headerselector);
        var bodywrapper = $(settings.bodywrapperselector);
        var body = $(settings.bodyselector);
        var row = $(settings.rowselector);
        var middlecol = $(settings.middlecolselector);
        var rightecol = $(settings.rightcolselector);
        ...
        if (module.length) {
            ...
            if (footer.length && $(window).width() < 1200) {
            ...
            /*var calculateHeaderFooter = function(){
                var parentOffset = module.offset();
                var parentArea = parentOffset.top + module.height();

                // HANDLE Fixed Header
                if (win.scrollTop() > parentOffset.top) {
                    module.addClass(settings.fixedheaderclass);
                } else {
                    module.removeClass(settings.fixedheaderclass);
                }

                // HANDLE Fixed Footer
                if (win.scrollTop() + win.height() > parentArea || win.scrollTop() + win.height() < parentOffset.top) {
                    module.removeClass(settings.fixedfooterclass);
                } else {
                    module.addClass(settings.fixedfooterclass);
                }
            }*/
            ...
            /*win.resize(function (event) {
                event.preventDefault();
                // Act on the event
                var fullWidth = module.width();
                var middleWidth = middlecol.width();
                //header.innerWidth(fullWidth - 2);
                //footer.innerWidth(fullWidth - 2);

                calculateHeaderFooter();
    ...
    initRelatedBlocks: function () {
        ...
        var relatedtarget = $(settings.relatectrlselector);
        var relatedlist = $(settings.relateditemselector);
        var relList = $(settings.relatectrllist);
        relatedlist.each(function (index, el) {
            var item = $(el);
            var wrapper = $(settings.relatectrlitemtemplatewrapper);

            var newbtn = $(settings.relatectrlitemtemplate);
            var icon = item.find(settings.relateditemiconselector).clone();
            var title = item.find(settings.relateditemtitleselector);

            var uniqid = 'relatedBlock' + chance.natural({min: (index + 1), max: 1000});

            item.attr('id', uniqid);
            newbtn.attr('href', '#' + uniqid);
            newbtn.attr('title', title.text());

            newbtn.html(title.html());
            wrapper.append(newbtn);
            wrapper.appendTo(relList);
        });

        relList.appendTo(relatedtarget);

        var collector = $(settings.collectorselector);
        var state = "normal";
        collector.each(function (index, el) {
            var item = $(el);
            if (item.is('[el-state]')) {
                state = item.attr('el-state');
            }
        });

        var footer = $(settings.footerselector);
        if (state == "initCL") {
            //footer.find('.previews_session').attr("disabled",false)
            //footer.find('.next_session').attr("disabled",true)
            //footer.find('.previews_session').show();
            //footer.find('.next_session').hide();
        } else if (state == "runnCL") {
            //footer.find('.previews_session').attr("disabled",true)
            //footer.find('.next_session').attr("disabled",true)
            //footer.find('.previews_session').hide();
            //footer.find('.next_session').hide();
        } else if (state == "lastCL") {
            //footer.find('.previews_session').attr("disabled",false)
            //footer.find('.next_session').attr("disabled","")

            //footer.find('.previews_session').hide();
            //footer.find('.next_session').show();
        } else {
            //footer.find('.previews_session').attr("disabled",false)
            //footer.find('.next_session').attr("disabled",false)
            //footer.find('.previews_session').show();
            //footer.find('.next_session').show();
        }
...
EW['profile-picture-upload'] = {
	dependencies: ['dropzone'],
    init: function(e){
		
			var accept = ".png,.jpg,.gif,.bmp,.jpeg";
	Dropzone.autoDiscover = false;
			 var uploadUrl = '' +$('.user_profile_picture').attr('data-upload-url');
			 console.log('aaa '+uploadUrl);
...
//		Dropzone.autoDiscover = false;
//
//		var profPictureObj = $('.user_profile_picture');
//		profPictureObj.each(function (index, el) {
//			 var uploadedItem = $(el);
//
//			if (uploadedItem.attr('data-allow') !== 'undefined' && uploadedItem.attr('data-allow') !== '') {
//                var allow_type = uploadedItem.attr('data-allow');
//            } else var allow_type = ".png,.jpg,.gif,.bmp,.jpeg";
//
//			 var uploadUrl = '' +uploadedItem.attr('data-upload-url');
//			 console.log('aaa',uploadedItem);
//
//			 console.log('url '+ uploadUrl);
//			// Dropzone class:
//			var userProfPictureDropzone = new Dropzone("#" + uploadedItem.attr('id'), { 
//				url: uploadUrl,
//				maxFilesize: 2000,
...
//    },
//				init: function() {
//					this.off('error').on('error', function(file, errorMessage) {
//					  var mypreview = document.getElementsByClassName('dz-error');
//					  mypreview = mypreview[mypreview.length - 1];
//					  mypreview.classList.toggle('dz-error');
//					  mypreview.classList.toggle('dz-success');
...
EW['app_doc_upload_img'] = {
	...
	if (document.getElementById('id_id_doc_upload'))
	   ...
	   {
	    var vl_id_id_doc_upload = document.getElementById('id_id_doc_upload').value;
	    id_doc_upload = '&id_sel=' + vl_id_id_doc_upload;
	   }

    var uploadUrl = '' + $('.app_doc_upload').attr('data-upload-url') + id_doc_upload;

    
    //console.log('aaa '+uploadUrl);
    var myDropzone = new Dropzone("#id_dropzone_doc_upload", { 
  	url: uploadUrl,
   	acceptedFiles: accept,
   	uploadMultiple: false,
   	createImageThumbnails: true,
...
EW["app_doc_upload"] = {
    ...
    init: function (e) {
        ...
        var dropObjMulti = $('.app_doc_upload');
        ...
        dropObjMulti.each(function (index, el) {
            ...
            var dropitemMulti = $(el);
                ...
                var dropitemMulti = $(el);
                if (dropitemMulti.attr('data-allow') !== '') {
                    var allow_type = dropitemMulti.attr('data-allow');
                } else{
                    var allow_type = "";
                }
	       ...
	       if (document.getElementById('id_id_doc_upload'))
            ...
            var url_small_multi = _ajx + dropitemMulti.attr('data-drop-url-multi');
            var myDropZoneMulti = new Dropzone('#' + dropitemMulti.attr('id'), {
                ...
                init: function () {
                    ...
                    var selector = '#' + EW["get-url"].parseurl(url_small_multi)[1];
                    ...
                    if (response) 
                       {
                        ...
                        var response_id_doc   = response_array[0];
                        var response_doc_name = response_array[1];
	                    
	                    if (document.getElementById('id_doc_is_upload'))
	                       {
                            document.getElementById('id_doc_is_upload').value = "Y";
                           }
 
                        var id_doc_upload = "";
	                    if (document.getElementById('id_id_doc_upload'))
	                       {
	                        document.getElementById('id_id_doc_upload').value = response_id_doc;
	                        id_doc_upload = '&id_sel=' + response_id_doc;
                           }

	                    if (document.getElementById('id_link_doc_name'))
	                       {
	                        document.getElementById('id_link_doc_name').innerHTML = response_doc_name;
                           }
                        ...
                        myDropZoneMulti.options.url = uploadUrl;
                        
                        if (dropitemMulti.is('[data-target]')) {

                            var target     = $(dropitemMulti.attr('data-target'));
